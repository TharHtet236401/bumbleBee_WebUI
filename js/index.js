...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const postDatas = await getPosts('http://127.0.0.1:3000/api/posts/getFeeds', token);
...
home_sidebar.addEventListener("click", () => {
    ...
    mainEl.innerHTML = `
        <div class="post_header_wrapper header_wrapper">
                ...
                <p id="create_post_btn" class="create_post_btn create_btn">Create Post</p>
            </div>
    ...
    console.log(posts, 'laklk')
    if (posts.length > 0) {
        posts.forEach((post) => {
            console.log(post, 'ggggg')
            const postHTMLEl = `
            <div class="post_card">
        ...
        posts_wrapper.innerHTML = '<p style="color: white;">No posts available</p>';
    }
...
create_post_form.addEventListener("submit", async (e) => {
    e.preventDefault();
    ...
    console.log(heading, body, contentType, contentPictures, documents);
...
function generatePostImages(postImageURLs, domEl) {
    ...
    if (postImageURLs.length > 0) {
        ...
        domEl.innerHTML = '<p style="color: white;">No posts available</p>';
    }
...
function generatePostFiles(postFilesArr) {
    ...
    if (postFilesArr.length > 0) {
        ...
        postFilesWrapper.innerHTML = '<p style="color: white;">No posts available</p>';
    }
